...
| 4 | WCS → 海柔 WES | 异步下发任务 | 失败按 1min 间隔重试 3 次（含首次共 4 次调用） |
| 4a | 分支：下发成功 | 海柔同步 ACK → WCS 状态置 `DISPATCHED` | 回填 `vendorTaskNo` |
| 4b | 分支：4 次调用均失败 | 状态置 `DISPATCH_FAILED` → 飞书告警 → 回传 WMS 下发失败 | 支持后台人工重推 |
...
![拣货出库主流程时序（含容器分配与库存不足）](images/03-seq-pick.png)

*图：拣货出库主流程时序（含容器分配与库存不足）（飞书粘贴 Markdown 时请手动插图，内容等价于下表）*
...
| 3 | WCS → 海柔 WES | 下发任务 | 失败按 1min×3 重试（共 4 次调用） |
...
| `wcs_task` | 1 : N | `wcs_task_result` | 结果回传流水（含分批，`vendor_msg_id` 幂等） |
...
![补货任务取消处理流程图](images/05-cancel.png)

*图：补货任务取消处理流程图（飞书粘贴 Markdown 时请手动插图，内容等价于下表）*
...
| 4c | ↳ 网络异常 / 超时 | 按 1min 间隔重试 3 次（含首次共 4 次调用）；成功走 4a | 4 次均失败 → 状态回滚 → 返回取消失败 | **是** |
