...
| 时间 | 版本 | 变更人 | 主要变更内容 |
|---|---|---|---|
| 2026-08-08 | v1.3 | Stone JIANG | **统一系统链路口径**：全文明确为「门店 WMS → WCS → 海柔 WES」三段式，**移除 WHC 这一中间系统概念**（名词解释、架构图、时序图、接口章节、协议信封命名全部更正）；新增 §7.7-R13，提请海柔将接口清单中的「WHC」统一更名为 WCS |
...
> **系统链路口径（全文统一）**：本项目的交互链路是 **门店 WMS → WCS → 海柔 WES**，共三段、三个系统，**不存在 WHC 这一独立中间系统**。
> 《台州飞箱（WCS-海柔接口）》接口清单中出现的 **「WHC」是历史称谓，对应的就是本项目的 WCS**；清单中「WMS→WHC→设备」即本文的「门店 WMS → WCS → 海柔 WES」。已提请海柔在清单中统一更名，见 §7.7-R13。
...
```
   ...
   链路只有三段：门店 WMS → WCS → 海柔 WES，WCS 是唯一中间层。
```
...
stateDiagram-v2
    ...
    DISPATCHING --> DISPATCH_FAILED: 共4次调用仍失败 → 飞书告警
...
> **链路口径**：本项目为 **门店 WMS → WCS → 海柔 WES** 三段式，没有 WHC 中间系统。接口清单沿用了「WHC」这一历史称谓，**它指的就是 WCS**——清单里的「WMS→WHC→设备」即本文的「门店 WMS → WCS → 海柔 WES」。
> 清单中标注为「WHC 接口访问地址」的列目前全部空白，**那正是 WCS 需要定义并回填给海柔与 WMS 的地址**，本节 §7.3 给出定义。
...
#### ③ 海柔 WES → WCS（**WCS 提供，需回填到接口清单中标注为「WHC 接口访问地址」的那一列**）
...
| **R11** | **海柔接口路径风格不一致**：`/WES/OPEN/...`（商品、上架、出库下发）、`/wrm/outbound/cancel`（出库取消）、`/plugin/stock/query/skuLot`（库存查询）；且接口 5/6/9/10/11/13/14 的地址栏为空 | WCS 配置项与联调准备 | 海柔/WCS | 海柔侧路径按实际配置到路由表即可；**海柔 WES → WCS 的 7 个接口地址由 WCS 定义（§7.3③），需回填到清单中标注为「WHC 接口访问地址」的那一列** |
| **R12** | **接口 2（商品信息拉取）明细页为空白**，P2 待定 | 1 期范围 | 产品/海柔 | 1 期不做，2 期再定义 |
| **R13** | **称谓不统一**：接口清单通篇使用「WHC」指代中间层，且流向写作「WMS→WHC→设备」。本项目实际链路为 **门店 WMS → WCS → 海柔 WES**，**没有 WHC 这个系统** | 三方沟通易产生「多了一层系统」的误解，联调时定位责任边界会混乱 | 海柔/WCS | 请海柔将清单中的「WHC」统一更名为「WCS」，流向统一写作「WMS→WCS→海柔 WES」；在更名完成前，本文按「WHC = WCS」对齐阅读 |
...
| 《海柔-WHC-FLUX接口文档 - Alex-Copy 0727》 | 海柔接口说明（文档标题中的 WHC 为历史称谓，对应本项目的 WCS） |
| 《飞箱库存快照》 | 料箱快照字段定义 |
