...
> **📌 关于本文件**
> 本文件是《【子PRD】台州海柔飞箱项目 — WCS 系统》的**飞书导入版**。
> 与标准 Markdown 版内容完全一致，仅将 4 张流程图/状态图改写为**表格与步骤清单**（飞书导入不渲染 mermaid）。
> **推荐导入方式**：飞书云文档 →「导入文档」→ 上传 `WCS子PRD_台州海柔飞箱项目.docx`，可保留全部图片与排版。
> 图片文件位于 `docs/wcs/images/`，可在飞书中手动插入到对应章节。
...
| 层级 | 参与方 / 组件 | 职责 |
|---|---|---|
| 上游 | 多仓店 WMS（台州门店 / 合肥门店 / FC / 云仓 / 大仓） | 发起作业指令，只对接 WCS 标准指令协议（协议不随厂商变化） |
| WCS-接入层 | WCS | 鉴权、幂等校验、参数校验、指令落库 |
| WCS-调度层 | WCS | 厂商路由、状态机、重试、告警 |
| WCS-适配层 | WCS | 厂商协议转换 Adapter（海柔 / 立镖 / Geek+ / 原力聚合） |
| 下游-设备 | 海柔 WES（1 期）；立镖、Geek+、原力聚合（预留） | 执行实际作业并回传结果 |
...
| 步骤 | 发起方 → 接收方 | 动作 | 说明 |
|---|---|---|---|
| 1 | WMS-库内 → WCS | 补货上架任务下发 | 接口 I-03 |
| 2 | WCS | 鉴权 → 幂等校验 → 参数校验 → 落库 | 状态置 `INIT` |
| 3 | WCS → WMS-库内 | 同步返回受理成功 + `wcsTaskNo` | 不等待厂商 |
| 4 | WCS → 海柔 WES | 异步下发任务 | 失败按 1min×3 重试 |
| 4a | 分支：下发成功 | 海柔同步 ACK → WCS 状态置 `DISPATCHED` | 回填 `vendorTaskNo` |
| 4b | 分支：3 次下发均失败 | 状态置 `DISPATCH_FAILED` → 飞书告警 → 回传 WMS 下发失败 | 支持后台人工重推 |
| 5 | 海柔 WES → WCS | 任务受理回执 | 接口 V-05 |
| 6 | WCS → WMS-库内 | 转发受理结果 | 接口 I-08 |
| 7 | 海柔 WES → WCS | 上架结果回传（可分批，带 `batchNo`） | 接口 V-06 |
| 8 | WCS | 按 `vendorMsgId` 幂等 → 明细累加 → 状态置 `EXECUTING` | 主表此时不置终态 |
| 9 | WCS → WMS-库内 | 每批实时转发上架结果 | 接口 I-09，不做合并等待 |
| 10 | 海柔 WES → WCS | 任务完结信号 `isFinished=true` | |
| 10a | 分支：全部上架 | 状态置 `SUCCESS` | |
| 10b | 分支：部分上架 | 状态置 `PARTIAL_DONE` → **飞书告警** | |
| 11 | WCS → WMS-库内 | 回传任务终态 | WMS 据此将未上架库存移回 SF |


### 3.3 主流程时序（拣货出库，含容器分配与库存不足）

**拣货出库端到端步骤（含容器分配与库存不足）：**

| 步骤 | 发起方 → 接收方 | 动作 | 说明 |
|---|---|---|---|
| 1 | WMS-出库 → WCS | 拣货出库任务下发 | 接口 I-05 |
| 2 | WCS → WMS-出库 | 同步受理 + `wcsTaskNo` | |
| 3 | WCS → 海柔 WES | 下发任务 | 失败按 1min×3 重试 |
| 4 | 海柔 WES → WCS | 受理回执 | 接口 V-05 |
| 5 | WCS → WMS-出库 | 转发受理结果 | 接口 I-08 |
| 6 | 海柔 WES → WCS | 容器分配结果 | 接口 V-07，可多次 |
| 7 | WCS → WMS-出库 | 转发容器分配结果 | 接口 I-10，WMS 据此完成作业 |
| 8 | 海柔 WES → WCS | （可选）出库前库存不足回传，分批带 `batchNo` | 接口 V-08 |
| 9 | WCS → WMS-出库 | 转发缺货明细 | 接口 I-11 |
| 10 | WCS → 飞书 | **告警：飞箱库存不足** | 5 分钟聚合 |
| 11 | 海柔 WES → WCS | 拣货结果回传（可分批） | 接口 V-06 |
| 12 | WCS → WMS-出库 | 转发拣货结果 | 接口 I-09 |
| 13 | 海柔 WES → WCS | 任务完结 | |
| 14 | WCS → WMS-出库 | 回传终态 `SUCCESS` / `PARTIAL_DONE`（部分拣货另发飞书告警） | |
...
| 4 | `DISPATCHING` | 厂商 ACK 成功 | `DISPATCHED` | 回填 `vendor_task_no` |
| 5 | `DISPATCHING` | 下发失败（网络/5xx/系统错） | `DISPATCHING` | 1 分钟后重试，最多 3 次 |
| 6 | `DISPATCHING` | 重试 3 次仍失败 | `DISPATCH_FAILED` | **飞书告警** + 回传 WMS |
...
| 16 | `CANCELLING` | 厂商取消失败 / 重试 3 次失败 | **回滚至取消前原状态** | **飞书告警** + 返回失败原因 |
...
| 2 | 状态为 `SUCCESS`/`FAILED`/`CANCELLED`/`CLOSED` | 状态不变 | 取消失败：任务已终态（`WCS_TASK_STATUS_ILLEGAL`）；若已 `CANCELLED` 则返回成功（幂等） | 否 |
| 3 | 状态为 `INIT`（尚未下发厂商） | 本地直接置 `CANCELLED` | 同步返回取消成功 | 否 |
| 4 | 状态为 `DISPATCHED`/`EXECUTING` | 置 `CANCELLING` 并记录原状态 → 同步调用厂商取消接口 | 视厂商结果，见 4a/4b/4c | — |
| 4a | ↳ 厂商返回取消成功 | 置 `CANCELLED`，未完成明细行置 `CANCELLED`，已完成行数量保留 | 同步返回取消成功 | 否 |
| 4b | ↳ 厂商返回业务失败 | **状态回滚至原状态** | 同步返回取消失败 + 厂商原文 | **是** |
| 4c | ↳ 网络异常 / 超时 | 按 1min×3 重试；重试成功走 4a | 3 次失败 → 状态回滚 → 返回取消失败 | **是** |
| 5 | 任务类型为移出 / 拣货 | 不调厂商 | `WCS_CANCEL_NOT_SUPPORT` | 否 |
